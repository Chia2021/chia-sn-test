...
import { mapContactMessageRow } from '../../lib/supabaseMappers';
...
export function ContactMessagesSection({ currentLang }: ContactMessagesSectionProps) {
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      setMessages((data ?? []).map(mapContactMessageRow));
    } catch (err: any) {
  ...
  const updateStatus = async (id: string, status: ContactMessageStatus) => {
    ...
    if (status === 'read' && !messages.find((m) => m.id === id)?.readAt) {
      patch.read_at = now;
    }
    if (status === 'replied') {
  ...
  const statusBadge = (status: ContactMessageStatus) => {
    ...
    return (
      <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-lg border ${item.cls}`}>
        {item.label}
      </span>
    );
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h4 className="font-bold text-sm text-slate-900 flex items-center gap-2">
            ...
            <span>{currentLang === 'FR' ? 'Messages & Demandes Clients' : 'Client Messages & Requests'}</span>
          </h4>
          <p className="text-xs text-slate-500 mt-1">
            {currentLang === 'FR'
              ? 'Consultez les demandes reçues via le formulaire de contact.'
              : 'Review requests received through the contact form.'}
          </p>
        </div>
        ...
        <button
          ...
          disabled={isLoading}
          className="inline-flex items-center gap-2 px-3 py-2 bg-white hover:bg-slate-50 text-slate-700 text-xs font-bold rounded-xl border border-slate-300 transition-colors disabled:opacity-50"
        >
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-2">
                    <a
                      href={`mailto:${msg.email}?subject=${encodeURIComponent(
                        currentLang === 'FR'
                          ? 'Re: Votre demande - Cabinet Chia-SN'
                          : 'Re: Your inquiry - Chia-SN Advisory'
                      )}`}
                      onClick={() => {
                    ...
                    {msg.status !== 'read' && (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-2 bg-blue-50 hover:bg-blue-100 text-blue-800 text-xs font-bold rounded-xl border border-blue-200 transition-colors"
                      >
                    ...
                    {msg.status !== 'archived' && (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl border border-slate-200 transition-colors"
                      >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-2 bg-red-50 hover:bg-red-100 text-red-700 text-xs font-bold rounded-xl border border-red-200 transition-colors ml-auto"
                    >
