...
export function TopBar({ currentLang, onToggleLang }: TopBarProps) {
  ...
  const adminLabel = isAdmin
    ? t['top-admin-btn-label-logged'] || 'CMS Panel'
    : t['top-admin-btn-label'] || 'Admin';

  const adminTooltip = isAdmin
    ? t['top-admin-btn-tooltip-logged'] || 'Open CMS admin panel'
    : t['top-admin-btn-tooltip'] || 'Admin Access (Manage text & images)';
    ...
    >
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-1.5 sm:gap-4">
        ...
        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 sm:gap-6 text-slate-200">
          <a
            id="top-email-link"
            href={`mailto:${topBarSettings.emailAddress}`}
          ...
          <a
            id="top-phone-link"
            href={`tel:${topBarSettings.phoneNumber.replace(/\s+/g, '')}`}
            className="inline-flex items-center gap-1.5 hover:text-amber-400 transition-colors"
          >
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          ...
          {topBarSettings.showAdminButton && (
            <button
              id="top-admin-btn"
              type="button"
          ...
          <div className="flex items-center gap-3 text-slate-300">
            {topBarSettings.showLinkedin && (
              <a
                id="social-linkedin"
                href={topBarSettings.linkedinUrl}
                target="_blank"
                rel="noopener noreferrer"
                aria-label={t['top-aria-linkedin'] || 'LinkedIn Chia-SN'}
            ...
            {topBarSettings.showFacebook && (
              <a
                id="social-facebook"
                href={topBarSettings.facebookUrl}
                target="_blank"
                rel="noopener noreferrer"
                aria-label={t['top-aria-facebook'] || 'Facebook Chia-SN'}
                className="hover:text-amber-400 transition-colors"
              >
                <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
            ...
            {topBarSettings.showWhatsapp && (
              <a
                id="social-whatsapp"
                href={`https://wa.me/${topBarSettings.whatsappNumber.replace(/\D/g, '')}`}
                target="_blank"
                rel="noopener noreferrer"
                aria-label={t['top-aria-whatsapp'] || 'WhatsApp Chia-SN Direct'}
